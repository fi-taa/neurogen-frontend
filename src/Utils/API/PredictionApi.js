...
export const Strokepredictor = async (data) => {
  const useData = await axios.post("https://neuronet.onrender.com/api/v1/predict/predict_stroke_risk", data, {
    headers: {
...
//   ]
// }
  const useData = await axios.post("https://neuronet.onrender.com/api/v1/predict/getStrokeRecommendations", data, {
    headers: {
      "Content-Type": "application/json",
    },
  });
...
export const StrokeAssementdata = async (data) => {
  const response = await axios.get(`https://neuronet.onrender.com/api/v1/predict/predictions/${data}`);
